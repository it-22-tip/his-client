...
export default {
  ...
  methods: {
    ...
    async transaction (transaction) {
      const { Villages } = this.$connection.models
      let page = this.activePage - 1
      let limit = 15
      let offset = page * limit
      let order = this.getOrder(Villages)
      ...
      let options = {
        ...
        attributes: [
          'Name',
          'Code'// ,
          // [this.$connection.literal('(SELECT COUNT(Id) FROM Villages WHERE Villages.DistrictCode = Villages.Code)'), 'Count']
        ],
        limit: limit,
        offset: offset,
        order: order,
      ...
      let rows = []
      try {
        rows = await Villages.findAll(options)
        options.raw = true
        options.attributes = undefined
        count = await Villages.count(options)
      } catch (error) {
        console.log(error)
      }
      console.log(rows)
