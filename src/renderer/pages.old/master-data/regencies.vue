...
export default {
  ...
  methods: {
    ...
    async transaction (transaction) {
      const { Regencies, Districts } = this.$connection.models
      let page = this.activePage - 1
      let limit = 15
      let offset = page * limit
      let order = this.getOrder(Regencies)
      ...
      let options = {
        ...
        attributes: [
          'Name',
          'Code',
          [this.$connection.literal('(SELECT COUNT(Id) FROM Districts WHERE Districts.RegencyCode = Regencies.Code)'), 'Count']
        ],
        limit: limit,
        offset: offset,
        order: order,
