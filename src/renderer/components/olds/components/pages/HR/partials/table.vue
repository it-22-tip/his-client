<template>
    <md-table class="hr-table" v-model="searched" md-elevation="0" md-sort="id" md-sort-order="asc" md-fixed-header>
      ...
      <md-table-row slot="md-table-row" slot-scope="{ item }" :class="item.class">
        <!-- <md-table-cell md-label="No" md-sort-by="id" md-numeric>{{ item.id }}</md-table-cell> -->
        <md-table-cell md-label="id" md-sort-by="id">{{ item.id }}</md-table-cell>
        <md-table-cell md-label="nip" md-sort-by="pre_nip">{{ item.pre_nip }}</md-table-cell>
        ...
        <md-table-cell md-label="">
          <md-button @click="clickEdit(item)" class="md-icon-button">
            <md-icon>edit</md-icon>
            <md-tooltip md-direction="top">Edit</md-tooltip>
          </md-button>
        </md-table-cell>
        <md-table-cell md-label="">
          <md-button v-if="item.warning" class="md-icon-button">
              ...
              employee.push(
                {
                  ...
                  warning: (e.id === 1 || e.id === 10) ? true : false,
                  class: []
                }
                )
            }
