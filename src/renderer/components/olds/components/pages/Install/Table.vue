<template>
  <md-content class="table-tabs">
    <md-content>
      <md-table v-model="tables" md-fixed-header>
      <md-table-toolbar>
        <div class="md-toolbar-section-start">
        <h1 class="md-title">All Table</h1>
        </div>
        <div class="md-toolbar-section-end">
          <md-button class="md-icon-button" @click="processInsert">
            Insert
             <md-tooltip md-direction="top">Recreate All</md-tooltip>
          </md-button>
          <md-button class="md-icon-button" @click="recreateAll">
        ...
        <md-table-row slot="md-table-row" slot-scope="{ item }">
          <md-table-cell md-label="Table" md-sort-by="name">{{ lowerCase(item.name) }}</md-table-cell>
          <md-table-cell md-label="">
            <md-button @click="clickAddRemove(item)" class="md-icon-button">
              <md-icon>{{ item.exists ? 'remove' : 'add' }}</md-icon><md-tooltip md-direction="top">{{ item.exists ? 'remove' : 'add' }}</md-tooltip>
            </md-button>
          </md-table-cell>
          <md-table-cell md-label="">
            <md-button @click="clickSeed(item)" class="md-icon-button">
              <md-icon>layers</md-icon>
              <md-tooltip md-direction="top">Seed Content</md-tooltip>
            </md-button>
          </md-table-cell>
          <md-table-cell md-label="">
            <md-button @click="clickTruncate(item)" class="md-icon-button">
    ...
    <md-snackbar :md-duration="3000" :md-active.sync="showSnackbar" md-persistent>
      ...
      <md-button class="md-primary" @click="toggleSnackbar">Retry</md-button>
    </md-snackbar>
  </md-content>
</template>
